...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        // Median Int
        MotionData imu_data_prev = imudata[i - 1];
        MotionData imu_data_curr = imudata[i];

        Eigen::Quaterniond dq;
        Eigen::Vector3d dtheta_half = (imu_data_prev.imu_gyro + imu_data_curr.imu_gyro) * dt / 4.;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        Eigen::Vector3d acc_w = Qwb * (dq * imu_data_curr.imu_gyro + imu_data_prev.imu_gyro) / 2. + gw;
        Qwb = Qwb * dq;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        Vw = Vw + dt * acc_w;

        save_points<<imu_data_curr.timestamp<<" "
            <<Qwb.w()<<" "
            <<Qwb.x()<<" "
            <<Qwb.y()<<" "
            <<Qwb.z()<<" "
